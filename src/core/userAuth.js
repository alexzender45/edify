...
async function generateAuthToken(payload) {
  const token = jwt.sign(payload, JWT_SECRETE_KEY, { expiresIn: TOKEN_DURATION });
  return token;
}
...
function decodeJwtToken(req) {
  ...
  const jwtPayload = jwt.verify(token, JWT_SECRETE_KEY, { expiresIn: TOKEN_DURATION });

  jwtPayload.token = token;
  return jwtPayload;
}

getUserPayload = async (payload)  => {
